...
async function mejorDesdeCatalogo(
  modo: ModoPescaGlobal,
...
async function mejorDeModo(opts: {
  modo: ModoPescaGlobal;
  favoritos: FavoritoZona[];
  puntos: PuntoGuardado[];
  actual?: { lat: number; lng: number; nombre: string } | null;
  ...
  if (!desdeCatalogo && (opts.modo === "orilla" || opts.modo === "barco") && opts.anclaCosta) {
    ...
    if (!misma) {
      desdeCatalogo = await mejorDesdeCatalogo(opts.modo, opts.anclaCosta);
    }
  }
  if (desdeCatalogo && (!mejor || desdeCatalogo.puntuacion > mejor.puntuacion)) {
    mejor = desdeCatalogo;
  }
 ...
 * Devuelve una fila por cada modo disponible siempre que el catálogo o
 * los sitios del usuario den candidatos (costa usa anclaCosta de respaldo).
 */
export async function elegirRecomendacionesHoyPack(opts: {
  ...
  ancla: { lat: number; lng: number };
  /** Centro costero provincial: respaldo para orilla/barco si el GPS es interior. */
  anclaCosta?: { lat: number; lng: number } | null;
  ...
  const resultados = await Promise.all(
    opts.modos.map((modo) =>
      mejorDeModo({
        ...
        coordsFavorito: opts.coordsFavorito,
        ancla: opts.ancla,
        anclaCosta: opts.anclaCosta,
      })
    )
  );

  // Mantener el orden de `modos` (río → orilla → barco).
